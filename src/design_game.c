...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/11 14:09:03 by aaqlzim           #+#    #+#             */
/*   Updated: 2020/02/24 16:17:12 by aaqlzim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_height(t_cub3d *cub, int index)
{
	...
	if (cub->side == 0)
		cub->perp_wd = (cub->block_x - cub->ppos_x +
		(1 - cub->step_x) / 2) / cub->ray_dir_x;
	else
		cub->perp_wd = (cub->block_y - cub->ppos_y +
		(1 - cub->step_y) / 2) / cub->ray_dir_y;
	if (cub->perp_wd == 0)
		cub->line_height = cub->resolution_y - 1;
	else
...
void	ft_dd_algo(t_cub3d *cub)
{
	...
	{
		cub->step_x = -1;
		cub->side_dist_x = (cub->ppos_x - cub->block_x) * cub->delta_x;
	}
	else
	{
		cub->step_x = 1;
		cub->side_dist_x = (cub->block_x + 1.0 - cub->ppos_x) * cub->delta_x;
	}
	if (cub->ray_dir_y < 0)
	{
		cub->step_y = -1;
		cub->side_dist_y = (cub->ppos_y - cub->block_y) * cub->delta_y;
	}
	else
	{
		cub->step_y = 1;
		cub->side_dist_y = (cub->block_y + 1.0 - cub->ppos_y) * cub->delta_y;
	}
}
...
void	ft_first_block(t_cub3d *cub)
{
	...
	while (cub->hit == 0)
	{
		if (cub->side_dist_x < cub->side_dist_y)
		{
			...
			cub->block_x += cub->step_x;
			cub->side = 0;
		}
		else
		{
			cub->side_dist_y += cub->delta_y;
			cub->block_y += cub->step_y;
			cub->side = 1;
		}
		if (cub->map[(int)cub->block_y][(int)cub->block_x] == '1')
			cub->hit = 1;
	}
...
int		ft_loop_game(t_cub3d *cub)
{
	...
	while (++i < cub->resolution_x - 1)
	{
		...
		cub->block_x = (int)cub->ppos_x;
		cub->block_y = (int)cub->ppos_y;
...
int		ft_make_game(t_cub3d *cub)
{
	...
	mlx_hook(cub->win_ptr, 17, 0, &ft_icon_close, cub);
